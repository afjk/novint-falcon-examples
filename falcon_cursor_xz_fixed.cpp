...
// マウス座標をFalcon座標に変換する関数（XZ軸版・Y軸固定）
std::pair<double, double> convertMouseToFalconXZ(CGPoint mousePos, CGSize screenSize) {
    // スクリーン座標系をFalcon座標系に変換
    // マウスX座標（0-screen width）をFalconX座標（-0.05～+0.05m）にマッピング
    double falconX = ((mousePos.x / screenSize.width) - 0.5) * 0.10;   // -0.05～+0.05m
...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "  マウスカーソル追従（XZ軸・Y軸固定版）" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    ...
    // 制御パラメータ（XZ軸版・Y軸固定）
    const double yFixed = 0.0;           // Y軸固定位置（中央）
    const double centeringForce = 120.0; // Y軸を中心に固定する力の強さ

    // PID制御パラメータ（XZ軸専用調整・振動抑制版）
    const double Kp = 90.0;              // 比例ゲイン（XZ軸用に調整）
    const double Ki = 5.0;               // 積分ゲイン（振動抑制のため低め）
    const double Kd = 8.0;               // 微分ゲイン（振動抑制）
    const double integralLimit = 0.005;  // 積分項の上限（厳しく制限）
    
    // ローパスフィルタパラメータ（振動除去用）
    const double alpha = 0.1;            // フィルタ係数（振動抑制重視）
    ...
    std::cout << std::endl;
    std::cout << "マウスカーソル追従制御を開始します（XZ軸版・Y軸固定）。" << std::endl;
    ...
    std::cout << "Y軸は" << (yFixed * 1000) << "mmに固定されます。" << std::endl;
    std::cout << "制御ゲイン: Kp=" << Kp << ", Ki=" << Ki << ", Kd=" << Kd << std::endl;
    std::cout << "ローパスフィルタ係数: " << alpha << " (振動抑制)" << std::endl;
    std::cout << "X可動範囲: ±50mm, Z可動範囲: 50-150mm" << std::endl;
