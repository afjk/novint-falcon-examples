...
10. **ざらざらテクスチャテスト（キューブ）** - キューブの壁に触れると、ざらざらした粗い表面の触感を体験できます
11. **もちもち球体テスト（お餅のようなぷにぷに）** - 柔らかくてもちもちした球体を触って、お餅のような触感を体験できます
12. **3Dメッシュ触覚フィードバック（OBJモデル）** - 任意の3DモデルをOBJ形式で読み込み、その形状を触って感じることができます
...
#### ざらざらテクスチャテスト
仮想的なキューブの壁に、ざらざらした粗い表面を再現します。壁をなぞると紙やすりのような触感を体験できます。
...
**特徴:**
- キューブの6面すべてにテクスチャを適用
- 複数のノイズ（周期的振動、ランダムノイズ、低周波ノイズ）を組み合わせてリアルな触感を実現
- テクスチャ強度: 0.3N

#### もちもち球体テスト
柔らかくてぷにぷにした球体を再現します。お餅のようなもちもちした触感を体験できます。
...
**特徴:**
- 半径40mmの柔らかい球体
- 低剛性（120 N/m）でとても柔らかい
- 程よい粘性（15 N·s/m）でお餅のような弾力
- 減衰力と非線形弾性でもちもち感を強化
- ゆっくり押すと柔らかく、深く押すと徐々に硬くなる

#### 3Dメッシュ触覚フィードバック
任意の3DモデルをOBJ形式で読み込み、その形状を触って感じることができます。

```bash
./run_falcon.sh 12 models/cube.obj 0.05
```

**使用方法:**
```bash
./run_falcon.sh 12 <OBJファイルパス> [スケール]
```

**パラメータ:**
- `<OBJファイルパス>`: 読み込むOBJファイルのパス（必須）
- `[スケール]`: モデルのスケール係数（オプション、デフォルト: 0.05）
  - 例: 0.05 = 50mm、0.1 = 100mm

**付属モデル:**
- `models/cube.obj`: シンプルな立方体
- `models/sphere.obj`: 球体（正二十面体近似）
- `models/pyramid.obj`: ピラミッド
...
# 球体を触る（80mmサイズ）
./run_falcon.sh 12 models/sphere.obj 0.08

# ピラミッドを触る（60mmサイズ）
./run_falcon.sh 12 models/pyramid.obj 0.06

# 独自のOBJモデルを読み込む
./run_falcon.sh 12 path/to/your/model.obj 0.1
```

**特徴:**
- Wavefront OBJ形式の3Dメッシュをサポート
- tinyobjloaderを使用した高速な読み込み
- 三角形メッシュとの正確な衝突判定
- 剛性: 800 N/m（しっかりとした硬さ）
- 最大力: 3 N（安全な力制限）
- ループ周波数: 1000 Hz（滑らかな触覚フィードバック）
- メッシュ表面からの最短距離を計算して自然な反力を生成
...
├── falcon_init_firmware.cpp    # ファームウェア初期化
├── falcon_texture_test.cpp     # ざらざらテクスチャテスト（キューブ）
├── falcon_viscous_sphere.cpp   # もちもち球体テスト（お餅のような触感）
├── falcon_mesh_haptic.cpp      # 3Dメッシュ触覚フィードバック（OBJローダー）
├── CMakeLists.txt              # ビルド設定
├── build/                      # ビルド出力（gitignore）
├── external/                   # 外部ライブラリ
│   └── tiny_obj_loader.h       # tinyobjloader（OBJファイル読み込み）
├── models/                     # 3Dモデルファイル
│   ├── cube.obj                # サンプル立方体
│   ├── sphere.obj              # サンプル球体
│   └── pyramid.obj             # サンプルピラミッド
